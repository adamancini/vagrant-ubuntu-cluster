...
sudo -E sh -c 'curl -k https://${UCP_IPADDR}/ca > /home/vagrant/ucp-ca.pem'
...
# Install DTR
sudo -E sh -c 'docker run --rm docker/dtr:${DTR_VERSION} install --ucp-url https://"${UCP_IPADDR}" --ucp-node dtr-node1 --replica-id "${DTR_REPLICA_ID}" --dtr-external-url https://dtr.landrush --ucp-username admin --ucp-password "${UCP_PASSWORD}" --ucp-ca "$(cat ucp-ca.pem)"'
# Run backup of DTR
...
sudo sh -c 'curl -k https://dtr.landrush/ca -o /usr/local/share/ca-certificates/dtr.landrush.crt'
